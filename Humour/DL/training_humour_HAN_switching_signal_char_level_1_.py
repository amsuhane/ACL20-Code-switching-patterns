...
with open('../Dataset/embedding_50d.pkl','rb') as F:
...
for i, (train, test) in enumerate(skf.split(padded_docs,labels)):
  ...
  if sys.argv[1]=="EnHi":
    ...
    if sys.argv[5]=="ReduceOne":    
      ...
      y = Dense(9, activation="relu", name='feature_vec')(inputB)
    else:
      y = Dense(9, activation="relu")(inputB)
...
with open(sys.argv[6],"a") as F:
  F.write(sys.argv[1]+"\t"+sys.argv[2]+"\t"+sys.argv[3]+"\t"+sys.argv[4]+"\t"+sys.argv[5]+"\t"+str(acc_sum/(n_folds*1.00))+"\t"+str(f1_sum/(n_folds*1.00))+"\n") 
